...
import '../../App.css';
import { constructReadingPlan, readingPlanValidateInputs } from '../../functions/readingPlanFunctions';


function Book() {

  const [additionalCovers, setAdditionalCovers] = useState([]);
  const [measure, setMeasure] = useState('chapter');
  ...
  const handlePlanConstruction = async (e) => {
    ...
    const result = constructReadingPlan(e, measure);
    setInSubmitMode(false);
  }
  ...
  return (
    ...
    <div className="flexBoxByCols ">
      ...
      <h2 className="book-main-details-text">Reading Plan</h2>
        ...
        :
        null}
      <p className="reading-plan-explanation">In the boxes below, first choose whether you want to read a set number of chapters or pages per day (ebooks can also help you track locations and percentage).  You also need to specify which page/chapter/etc you are current at or intend to start reading from as well as the last page/chapter/etc.  Then let us know how many pages/chapters/etc you'll be reading each day or when you'd like to have finished reading by.  We'll then generate a plan for you to track your progress.</p>
      <div className="flexBoxByRows form-section">
        <div className="flexBoxByCols radio-column">
          ...
          <div className="radio-row">
            <input type="radio" id="Chapters" name="meausurement" value="Chapters" checked onClick={()=>setMeasure('chapter')}/>
            <label for="Chapters">Chapters</label>
          </div>
          <div className="radio-row">
            <input type="radio" id="Pages" name="meausurement" value="Pages" onClick={()=>setMeasure('page')}/>
